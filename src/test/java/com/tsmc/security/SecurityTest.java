...
public class SecurityTest {
  ...
  public void testOauthToken() {
    ...
    ResponseEntity<Map> response =
        this.getOauthToken(clientid, clientpw, username, password, granttype, scope);
  ...
  public void testRefreshToken() {
    ...
    String scope = "account role";
    ResponseEntity<Map> response =
        this.getOauthToken(clientid, clientpw, username, password, "password", scope);
  ...
  // 使用 TestRestTemplate
  private ResponseEntity<Map> getOauthToken(String clientid, String clientpw, String username,
      String password, String granttype, String scope) {
    
    System.out.println(clientid + " " + clientpw);
    
    TestRestTemplate basicAuthTemplate = testRestTemplate.withBasicAuth(clientid, clientpw);
    ...
    System.out.println(username + " " + password + " " + granttype + " " + scope);
    
    HttpEntity<MultiValueMap<String, String>> entity =
        new HttpEntity<MultiValueMap<String, String>>(map, headers);
    ResponseEntity<Map> response =
        basicAuthTemplate.exchange("/oauth/token", HttpMethod.POST, entity, Map.class);
    return response;
  }

  // 更新 token
  private ResponseEntity<Map> refreshNewToken(String clientid, String clientpw, String granttype,
      String refreshtoken) {
    TestRestTemplate basicAuthTemplate = testRestTemplate.withBasicAuth(clientid, clientpw);
    HttpHeaders headers = new HttpHeaders();
    headers.setContentType(MediaType.APPLICATION_FORM_URLENCODED);
  ...
  // 自己配置比較多的細節
  private ResponseEntity<Map> getOauthTokenCust(String clientid, String clientpw, String username,
      String password, String granttype, String scope) {
    ...
    HttpEntity<MultiValueMap<String, String>> entity =
        new HttpEntity<MultiValueMap<String, String>>(map, headers);
    ResponseEntity<Map> response =
        testRestTemplate.exchange("/oauth/token", HttpMethod.POST, entity, Map.class);
    return response;
  }
  ...
  // "/api/v1/ItemGroupReport?id={id}&itemGroupName={itemGroupName}&shopIdPmt={shopIdPmt}&brand={brand}&isSpecial={isSpecial}",
  // byte[].class,
